...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        # remove gaussians below the threshold
        tmp = self._mask.detach().clone()
        thresh = 0.1*tmp.max()
        mask = self._mask > max(thresh, 0)
        mask = mask.squeeze()
    ...
    def save_ply_mask(self, path, mask):
        mkdir_p(os.path.dirname(path))
        self._xyz = self._xyz[mask]
        self._mask = self._mask[mask]
        self._features_dc = self._features_dc[mask]
