...
parser.add_argument("--save_path", type=str, default="/data/SAFIRE/bicycle/teddybear", help="save dir.")
parser.add_argument("--input_path", type=str, default="/data/tamper/teddybear/images", help="input dir.")
...
def main():
    ...
    test_forensic_datasets = {
        "Arbitrary": FE.data.Dataset_Arbitrary(Path(args.input_path)),
    }
    ...
    for dataset_name, test_forensic_dataset in test_forensic_datasets.items():
        ...
        with torch.no_grad():
            for step, (image, gt2D, img_paths) in enumerate(
                ...
                safire_pred = ((safire_pred>0.5) * 255).astype(np.uint8)
                safire_pred = Image.fromarray(safire_pred)
                safire_pred = safire_pred.resize((w,h))
                safire_pred.save(save_dir / (test_forensic_dataset.get_filename(step)))
